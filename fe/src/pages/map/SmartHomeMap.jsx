...
import { 
  ...
  MousePointer2, Droplets, Radar, Video, Wifi, RefreshCw, Zap, Power, AlertTriangle, CheckCircle2,
  Crosshair, Copy, Check
} from 'lucide-react';
...
// ================= CẤU HÌNH 15 BLOCK RADAR (Tính theo %) =================
const RADAR_BLOCKS = [
  ...
  { id: 13, left: 47.27, top: 54.03, width: 5.56, height: 11.26 }, // Khối lớn
...
const HALLWAY_RADAR_BLOCKS = [
  {
    ...
    width: 4.25,
    height: 12.57,
    clipPath: 'polygon(20.94% 96.10%, 92.00% 100%, 100% 0%, 0% 1.99%)'
  },
  {
    id: 'hallway_2',
    label: 'H2',
    ...
    width: 7.00,
    height: 15.85,
    clipPath: 'polygon(0% 0.76%, 6.86% 100%, 87.29% 98.49%, 100% 0%)'
  }
];
...
export default function SmartHomeMap() {
  ...
  const [timeFilter, setTimeFilter] = useState('1D'); // '1D', '3D'
...
const showRadarTarget = (radarName, blockId) => {
  ...
  // Nếu radar này đã có timer cũ thì xóa timer cũ
  if (radarTimersRef.current[radarName]) {
    clearTimeout(radarTimersRef.current[radarName]);
  }

  // Chỉ cập nhật block của radar hiện tại, không xóa radar khác
  setRadarTargets(prev => ({
    ...prev,
    [radarName]: blockId
  }));

  // Sau 3 giây chỉ xóa block của radar hiện tại
...
if (updatedDevice.deviceType === 'radar') {
  ...
  // Lắng nghe thay đổi ID thiết bị, Tab hoặc Filter thời gian để gọi API
  useEffect(() => {
  ...
  const handleMapClick = (e) => {
    ...
    setPickedPos({ x: (((e.clientX - rect.left) / rect.width) * 100).toFixed(2), y: (((e.clientY - rect.top) / rect.height) * 100).toFixed(2) });
    setCopied(false);
  };
        ...
        <TransformWrapper centerOnInit={false} initialScale={1} initialPositionX={-50} initialPositionY={50} minScale={0.3} maxScale={5} wheel={{ step: 0.001, smoothStep: 1 }} pinch={{ step: 1 }} limitToBounds={false}>
          {({ zoomIn, zoomOut, resetTransform }) => (
            <>
              ...
              <div className="absolute bottom-8 left-[40%] -translate-x-1/2 z-30 flex items-center gap-4">
                <div className="flex items-center gap-2 bg-slate-900/80 backdrop-blur-md p-2 rounded-2xl border border-white/10 shadow-2xl">
                  <button onClick={() => { setIsEditMode(!isEditMode); setPickedPos(null); setSelectedSensor(null); }} className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all font-bold text-xs uppercase tracking-widest ${isEditMode ? 'bg-fuchsia-600 text-white shadow-[0_0_15px_#c026d3]' : 'text-slate-400 hover:text-white hover:bg-white/10'}`}>
                    <Crosshair className="w-4 h-4" /> {isEditMode ? 'Đang Lấy Tọa Độ' : 'Chấm Vị Trí'}
                  </button>
                  {isEditMode && pickedPos && (
                    <div className="flex items-center gap-3 px-3 py-1.5 border-l border-white/10">
                      <div className="text-xs font-mono text-fuchsia-300">X: {pickedPos.x}% | Y: {pickedPos.y}%</div>
                      <button onClick={() => { navigator.clipboard.writeText(`"pos2dX": ${pickedPos.x}, "pos2dY": ${pickedPos.y}`); setCopied(true); setTimeout(() => setCopied(false), 2000); }} className="p-1.5 bg-white/10 hover:bg-white/20 rounded-lg transition-colors text-white">
              ...
              <TransformComponent wrapperClass="!w-screen !h-screen" contentClass={`w-full h-full flex items-center justify-center ${isEditMode ? 'cursor-crosshair' : 'cursor-grab active:cursor-grabbing'}`}>
                <div className="relative inline-block max-w-[90vw] max-h-[90vh]">
                  
                  <div onClick={handleMapClick} className="relative">
  ...
  return (
    <div
      ...
      className={`absolute border transition-all duration-300 flex items-center justify-center ${
        isActive
          ? 'bg-rose-500/40 border-rose-400 shadow-[0_0_20px_rgba(244,63,94,0.6)] z-20 backdrop-blur-[1px]'
          : 'bg-indigo-500/5 border-indigo-500/10 z-0'
      }`}
      style={{
        left: `${block.left}%`,
        top: `${block.top}%`,
    ...
    >
      {isActive && <div className="w-2.5 h-2.5 bg-white rounded-full animate-ping"></div>}
                    ...
                    {isEditMode && pickedPos && (
                      <div className="absolute w-5 h-5 bg-fuchsia-500/30 rounded-full border-2 border-fuchsia-400 -translate-x-1/2 -translate-y-1/2 flex items-center justify-center z-50 pointer-events-none shadow-[0_0_15px_#c026d3]" style={{ left: `${pickedPos.x}%`, top: `${pickedPos.y}%` }}>
                  ...
                  {filteredSensors.map((sensor) => {
                    ...
                    return (
                      <button
                        ...
                        className={`absolute -translate-x-1/2 -translate-y-1/2 transition-all duration-300 z-10 group ${isEditMode ? 'pointer-events-none opacity-30' : 'hover:scale-150 hover:z-20'} ${sensor.isFake ? 'opacity-60 grayscale hover:grayscale-0' : ''}`}
                      >
                        {hasWarning && !isEditMode && (
                          <div className="absolute inset-0 rounded-full animate-ping opacity-75 bg-rose-500"></div>
                        )}

                        <div className={`relative flex items-center justify-center w-6 h-6 md:w-8 md:h-8 rounded-full text-white shadow-2xl border-2 ${isSelected && !isEditMode ? 'border-white ring-4 ring-white/30' : 'border-white/60'} ${hasWarning ? 'bg-rose-600' : getTypeColor(sensor.deviceType)}`}>
  ...
  // Đèn / rèm / đồ điện: chỉ hiện lịch sử hoạt động
  <button
  ...
  // Cảm biến môi trường / an toàn: chỉ hiện lịch sử cảnh báo
  <button
  ...
  // Các thiết bị khác giữ nguyên cả lịch sử và cảnh báo
  <>
    <button
      onClick={() => handleTabClick('history')}
      className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 text-[10px] font-bold uppercase tracking-widest rounded-lg transition-all ${
